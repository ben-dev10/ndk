...
function replaceRegistryPaths(inputStr: string): string {
  return inputStr.replace(/(['"])([\s\S]*?)\1/g, (match, quote, content) => {
    ...
    // 🟡
      ...
      const rest = content.slice("@_ndk/ui/".length);
      return `${quote}@/${rest}${quote}`;
    } else if (content.startsWith("@_ndk/motion/")) {
      const rest = content.slice("@_ndk/motion/".length);
      return `${quote}@/motion/${rest}${quote}`;
    }
    return match;
  });
}
...
async function buildRegistry() {
  ...
  await new Promise((resolve, reject) => {
    const process = exec(
      // 🟡
      `npx shadcn@latest build public/r/registry.json --output ./public/r/`,
    );
  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      // 🟡 Replace `@/registry` and other internal imports in file contents
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      registryItem.files = registryItem.files?.map((file: any) => {
        if (
          file.content?.includes("@/registry") ||
          file.content?.includes("@_ndk/ui/") ||
          file.content?.includes("@_ndk/motion/")
        ) {
